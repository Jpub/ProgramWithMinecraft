...
def removeWater(structure):
    for row in range(len(structure)):
        for column in range(len(structure[row])):
            for depth in range(len(structure[row][column])):
                block = structure[row][column][depth]
                if block == 8 or block == 9:
                    structure[row][column][depth] = 0
    return structure
...
# remove the water
structure = removeWater(structure)
